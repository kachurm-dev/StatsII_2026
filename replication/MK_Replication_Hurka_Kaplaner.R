...
setwd("~/Documents/08. PhD Work/03. Classes/2026 Quants II HT /Replication")


install.packages(c("overlapping", "truncnorm", "dotwhisker", "broom", "gridExtra"))
...
boundaries_lrgen <- c(0, 10) #lrgen is coded from 0 to 10
boundaries_position <- c(1, 7) #position is coded from 1 to 7
...
reg_base$powerful <- as.factor(reg_base$powerful)

model_interaction <- lm(overlap ~ popularity_rank * powerful, data = reg_base)
summary(model_interaction)

ggplot(reg_base, aes(x = popularity_rank, y = overlap, color = powerful)) +
  geom_point(size = 3) +
  geom_smooth(method = "lm", se = FALSE) +
  theme_classic() +
  labs(
    x = "Committee popularity rank",
    y = "Committee representativeness (overlap)",
    color = "Committee power",
    title = "Interaction between committee popularity and power"
  )
